...
void
flashcache_dirty_writeback_range(struct cache_c *dmc, sector_t start_block, int len)
{
	int i;
	for(i = start_block; i < start_block+len; i++)
	{
		struct cacheblock *cacheblk = &dmc->cache[i];
		if (cacheblk->cache_state & DIRTY)
		{
			cacheblk->cache_state |= DISKWRITEINPROG;
			flashcache_dirty_writeback(dmc, i);
		}
	}
}
...
static void
flashcache_purge_blocks(struct cache_c *dmc, sector_t start_block, int num_sectors)
{
	int set = hash_block(dmc, start_block);
	int start_index, end_index, i;
	struct cacheblock *cacheblk;
	...
	for (i = start_index ; i < end_index ; i++) {
		...
		if (cacheblk->cache_state & INVALID)
			continue;
		if (cacheblk->dbn < start_block || 
		    cacheblk->dbn >= (start_block + num_sectors) ||
		    (cacheblk->cache_state & (DIRTY | BLOCK_IO_INPROG)) ||
		    cacheblk->nr_queued > 0)
			continue;
		/* We have a match and the block is not undergoing IO */
		dmc->cached_blocks--;
		cacheblk->cache_state = INVALID;
	}
}
...
void
flashcache_purge_block_range(struct cache_c *dmc, sector_t dbn, int num_sectors)
{
	unsigned long flags;
	int set;
	int num_blocks_to_purge = 0;
	int start_block;
	
	spin_lock_irqsave(&dmc->cache_spin_lock, flags);
	start_block = dbn;
	set = hash_block(dmc, dbn);
	/* Break the input range into sub-ranges that map onto sets */
	while (num_sectors >= dmc->block_size) {
		if (hash_block(dmc, dbn) == set) {
			/* Block consumed within current subrange */
			num_blocks_to_purge += dmc->block_size;
			dbn += dmc->block_size;
			num_sectors -= dmc->block_size;
		} else {
			/* New subrange starts, flush old subrange */
			VERIFY(num_blocks_to_purge > 0);
			flashcache_purge_blocks(dmc, start_block, num_blocks_to_purge);
			num_blocks_to_purge = 0;
			start_block = dbn;
			set = hash_block(dmc, dbn);
		}
	}
