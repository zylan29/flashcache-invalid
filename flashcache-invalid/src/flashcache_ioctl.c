...
#endif
{
	...
	pid_t pid;
	struct flashcache_trim_block_range trim;

	switch(cmd) {
	case FLASHCACHE_TRIM_BLK_RANGE:
		if (copy_from_user(&trim, (struct flashcache_trim_block_range *)arg, 
				   sizeof(struct flashcache_trim_block_range)))
			return -EFAULT;
		flashcache_dirty_writeback_range(dmc, trim.start_dbn>>3, trim.num_sectors>>3);
		flashcache_purge_block_range(dmc, trim.start_dbn, trim.num_sectors);
		return 0;
