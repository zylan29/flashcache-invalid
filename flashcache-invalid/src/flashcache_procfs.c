...
//-----------------------------------  BLOCK   STAT  -----------------------------------------
static int 
flashcache_blockstat(struct seq_file *seq, void *v)
{
	struct cache_c *dmc = seq->private;
	int i, valid, invalid, dirty;

	valid = invalid = dirty = 0;
	...
	seq_printf(seq, "blocksize = %d kb\n", dmc->block_size/2);
	seq_printf(seq, "num_sets = %d\n", dmc->num_sets);
	seq_printf(seq, "----------------------------------------------------------\n");
	seq_printf(seq, "block_num = %lu\n", dmc->size);
	seq_printf(seq, "dirty_blocks = %d\n", dirty);
	seq_printf(seq, "valid_blocks = %d\n", valid);
	seq_printf(seq, "invalid_blocks = %d\n", invalid);
	return 0;
}

static int 
flashcache_blockstat_open(struct inode *inode, struct file *file)
{
	return single_open(file, &flashcache_blockstat, PDE(inode)->data);
}

static struct file_operations flashcache_blockstat_operations = {
	.open		= flashcache_blockstat_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
//-----------------------------------  BLOCK  STAT   -----------------------------------------
	s = flashcache_cons_procfs_cachename(dmc, "flashcache_blockstat");
	entry = create_proc_entry(s, 0, NULL);
	if (entry) {
		entry->proc_fops =  &flashcache_blockstat_operations;
		entry->data = dmc;
	}
	kfree(s);
//-----------------------------------  BLOCK  STAT   -----------------------------------------
...
//-----------------------------------  BLOCK   STAT-----------------------------------------
	s = flashcache_cons_procfs_cachename(dmc, "flashcache_blockstat_hist");
	remove_proc_entry(s, NULL);
	kfree(s);
//-----------------------------------  BLOCK   STAT-----------------------------------------
